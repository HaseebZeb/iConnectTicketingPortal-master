...
namespace BasketWebPanel.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        public ActionResult ForgotPasswordSendEmail()
        {
            return View(new ForgotViewModelSendEmail());
        }

        [AllowAnonymous]
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ForgotPasswordSendEmail(ForgotViewModelSendEmail model)
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            }
            var response = AsyncHelpers.RunSync<JObject>(() => ApiCall.CallApi("api/User/ResetPasswordThroughEmailAdmin", User,null,true,false,null,"Email="+ model.Email));


            if (response == null || response is Error)
            {
                ModelState.AddModelError("", (response as Error).ErrorMessage);
                return View("ForgotPasswordSendEmail", model);
            }
                //return new HttpStatusCodeResult(HttpStatusCode.InternalServerError, (response as Error).ErrorMessage);
            else
                return RedirectToAction("ForgotPasswordSendEmailSuccess");
        }

        [AllowAnonymous]
        public ActionResult ForgotPasswordSendEmailSuccess()
        {
            return View();
        }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            try
            {
                ...
                {
                    var adminModel = response.GetValue("result").ToObject<UserViewModel>();
                    ...
                    identity.AddClaim(new Claim("access_token", adminModel.Token));
                    //identity.AddClaim(new Claim("token_type", adminModel.token_type));
